...
# of the address string into new strings:
...
# The highlight_word function changes the given
#  word in a sentence to its upper-case version.
...
# a list of squared numbers (n*n). 
# The function receives the variables start and end, 
# and returns a list of squares of consecutive 
# numbers between start and end inclusively. 
# For example, squares(2, 3) 
# should return [4, 9].
...
# and values of the car_prices dictionary, 
# printing out some information about each one.

def car_listing(car_prices):
    result = ""
    for model, price in car_prices.items():
        result += "{m} costs {p} dollars".format(m=model, p=price) + "\n"
    return result
...
# and the number of guests from 
# Rory's dictionary taking precedence, 
...
def combine_lists_2(list1, list2):
    # Generate a new list containing the elements of list2
    # Followed by the elements of list1 in reverse order
    l2 = list2.copy()
    l2.extend(reversed(list1))
    return l2
...
print(combine_lists_2(Jamies_list, Drews_list))
